...
class SubscriptionViewModel(
    ...
    fun load() {
        val items = when ("subscription") {
            ...
            else -> listOf("Subscription ready", "Fake data only", "No Firebase, AI, Room, Retrofit, or network")
        }
        uiState = if (items.isEmpty()) SubscriptionUiState.Empty else SubscriptionUiState.Success(items)
    }
}