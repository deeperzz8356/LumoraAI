...
class CreditsViewModel(
    ...
    fun load() {
        val items = when ("credits") {
            ...
            else -> listOf("Credits ready", "Fake data only", "No Firebase, AI, Room, Retrofit, or network")
        }
        uiState = if (items.isEmpty()) CreditsUiState.Empty else CreditsUiState.Success(items)
    }
}