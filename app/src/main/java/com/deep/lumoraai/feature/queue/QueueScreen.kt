...
import com.deep.lumoraai.feature.queue.components.QueueFeatureCard
...
fun QueueScreen(
    ...
    Scaffold(
        ...
        when (uiState) {
            QueueUiState.Loading -> Loading(modifier = Modifier.padding(padding))
            QueueUiState.Empty -> EmptyState(title = "Queue", message = "No fake data yet.", modifier = Modifier.padding(padding))
            is QueueUiState.Error -> ErrorState(title = "Queue", message = uiState.message, modifier = Modifier.padding(padding))
            is QueueUiState.Success -> QueueContent(items = uiState.items, onNext = onNext, modifier = Modifier.padding(padding))
        }
    }
}

@Composable
private fun QueueContent(items: List<String>, onNext: () -> Unit, modifier: Modifier = Modifier) {
    Column(modifier = modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        items.forEach { item -> QueueFeatureCard(title = item, subtitle = "Local fake data") }
