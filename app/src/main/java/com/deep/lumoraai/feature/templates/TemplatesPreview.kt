...
private val previewState = TemplatesUiState.Success(listOf("Templates fake item", "Preview data", "Compile-only screen"))

@Preview(name = "Templates Light Preview", showBackground = true)
@Composable
fun TemplatesLightPreview() {
    LumoraTheme(darkTheme = false) { TemplatesScreen(uiState = previewState, onNext = {}) }
}

@Preview(name = "Templates Dark Preview", showBackground = true)
@Composable
fun TemplatesDarkPreview() {
    LumoraTheme(darkTheme = true) { TemplatesScreen(uiState = previewState, onNext = {}) }
}
