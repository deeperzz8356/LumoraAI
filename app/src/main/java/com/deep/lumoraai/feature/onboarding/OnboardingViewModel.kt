...
class OnboardingViewModel(
    ...
    var uiState: OnboardingUiState by mutableStateOf(OnboardingUiState.Loading)
        private set

    init { load() }

    fun load() {
        val items = when ("onboarding") {
            ...
            else -> listOf("Onboarding ready", "Fake data only", "No Firebase, AI, Room, Retrofit, or network")
        }
        uiState = if (items.isEmpty()) OnboardingUiState.Empty else OnboardingUiState.Success(items)
    }
}