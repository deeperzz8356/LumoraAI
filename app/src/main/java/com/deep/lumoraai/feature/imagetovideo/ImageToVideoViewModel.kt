...
class ImageToVideoViewModel(
    ...
    var uiState: ImageToVideoUiState by mutableStateOf(ImageToVideoUiState.Loading)
        private set

    init { load() }

    fun load() {
        val items = when ("imagetovideo") {
            ...
            else -> listOf("Image To Video ready", "Fake data only", "No Firebase, AI, Room, Retrofit, or network")
        }
        uiState = if (items.isEmpty()) ImageToVideoUiState.Empty else ImageToVideoUiState.Success(items)
    }
}